import {
  ...
  export class AiDeepSearchController {
    ...
    @Post()
    @ApiOperation({ summary: 'Create a deep-search record' })
    create(
      @Req() req,
      @Body() dto: CreateAiDeepSearchDto,
    ) {
    ...
    @ApiQuery({ name: 'perPage', required: false, type: Number, example: 10, description: 'Items per page (default: 10)' })
    findAll(
      @Query('page') page = '1',
    ...
    @Patch(':id')
    @ApiOperation({ summary: 'Update a deep-search record' })
    update(
      @Param('id') id: string,
      @Body() dto: UpdateAiDeepSearchDto,
    ) {
      return this.svc.update(id, dto);
    }
  
    @Delete(':id')
    @ApiOperation({ summary: 'Delete a deep-search record' })
    remove(
      @Param('id') id: string,
    ) {
